...
/**
 * http://www.aichengxu.com/java/32152.htm
 */
...
public class ThreadManager {
    ...
    /**
     * 获取一个用于执行长耗时任务的线程池，避免和短耗时任务处在同一个队列，而阻塞了重要的短耗时任务，通常用联网操作
     */
    ...
    /**
     * 获取一个用于执行短耗时任务的线程池，避免因为和耗时长的任务处在同一个队列儿长时间得不到执行，通常用来执行本地的IO/SQL
     */
     ...
     */
    public static ThreadPoolProxy getDefaultSinglePool(){
    ...
    public static class ThreadPoolProxy{
        ...
        public synchronized void execute(Runnable runn){
            ...
            if (mPool == null || mPool.isShutdown()) {
                //参数说明
                //当线程池中的线程小于mCorePoolSize，直接出创建新的线程加入线程池执行任务
                //当线程池中的线程数目等于mCorePoolSize,将会把任务加入到任务队列BlockingQueue中
                //当BlockingQueue中的任务放满了，安静会创建新的线程去执行
                //但是当总线程数大于mMaximumPoolSize时，将会抛出异常，交给RejectedExecutionHandler处理
                //mKeepAliveTime是线程执行完成后，且队列中没有可以执行的任务，存活的时间，后面的参数是时间单位
                //ThreadFactory是每次其创建新的线程工厂
                mPool=new ThreadPoolExecutor(mCorePoolSize,mMaximumPoolSize,mKeepAliveTime, TimeUnit.MILLISECONDS,new LinkedBlockingDeque<Runnable>(), Executors.defaultThreadFactory(),new ThreadPoolExecutor.AbortPolicy());
            }
            mPool.execute(runn);
        }
