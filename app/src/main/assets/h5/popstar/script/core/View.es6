...
import {popStarObserver} from '../externalGame.es6'
...
export default class view {
  constructor (view) {
    ...
    this.level.set({top: 16, right: 16});
    ...
    // 队友分数
    this.partnerScoreLabel = new PIXI.Text('队友分数：0', {
        fontFamily: "Arial",
        fontSize: 16,
        fill: 0xf7f408,
        align: "left"
    ...
    // 总分数
    this.totalScoreLabel = new PIXI.Text('总分：0', {
        fontFamily: "Arial",
        fontSize: 16,
        fill: 0xf7f408,
        align: "left"
    ...
    Reflect.defineProperty(this, "partnerScore",
      {
        get: () => this._partnerScore || 0,
        set: value => {
          this._partnerScore = value;
          this.partnerScoreLabel.text = `队友分数：${value}`
          this.totalScore = this.total + this.partnerScore
          // this.totalScoreLabel.text = `总分：${Number(this.partnerScore) + value}`
        }
      }
    );

    Reflect.defineProperty(this, "totalScore",
      {
        get: () => this._totalScore || 0,
        set: value => {
          this._totalScore = value;
          this.totalScoreLabel.text = `总分：${Number(this._totalScore)}`
        }
      }
    );
    ...
    this.resurgenceLabel = new PIXI.Text('复活次数：0', {
        fontFamily: "Arial",
        fontSize: 16,
        fill: 0xf7f408,
        align: "left"
    ...
    Reflect.defineProperty(this, "resurgenceNum",
      {
        get: () => this._resurgenceNum || 0,
        set: value => {
          this._resurgenceNum = value;
          this.resurgenceLabel.text = `复活次数：${value}`
        }
      }
    );
    // 消除信息
    ...
    this.game.addChild(
      ...
      this.goal,
      this.totalLabel,
      this.partnerScoreLabel,
      this.totalScoreLabel,
      this.cleanInfo,
      this.bountyLabel,
      this.starInfo,
      this.resurgenceLabel
    );
    ...
    TweenMax.to(
      ...
      .4,
      {
        ...
        onComplete: () => {
          ...
          // this.partnerScore += tile.score
          // this.totalScore += tile.score
          popStarObserver.next({total: true})
        }
      }
    )
