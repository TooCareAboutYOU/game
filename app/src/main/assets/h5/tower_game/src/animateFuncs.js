...
export const endAnimate = (engine) => {
  ...
  drawYellowString(engine, {
    string: `总分 ${engine.partnerScore + gameScore}`,
    size: engine.width * 0.05,
    x: (engine.width * 0.1) + threeFiguresOffset,
    y: engine.width * 0.3,
    textAlign: 'left'
  })
  drawYellowString(engine, {
    string: `队友得分 ${engine.partnerScore}`,
    size: engine.width * 0.05,
    x: (engine.width * 0.1) + threeFiguresOffset,
    y: engine.width * 0.4,
    textAlign: 'left'
  ...
  for (let i = 1; i <= 7; i += 1) {
    ...
    ctx.drawImage(
      heart,
      (engine.width * 0.66) + ((i - 1 - 3) * zoomedHeartWidth - zoomedHeartWidth*0.4),
      engine.width * 0.16,
      zoomedHeartWidth,
      zoomedHeartHeight
    )
  ...
  //   string: 'x7',
  //   size: engine.width * 0.06,
  //   x: (engine.width * 0.66) + (2 * zoomedHeartWidth),
  //   y: engine.width * 0.16 + (0.66 * zoomedHeartHeight),
  //   textAlign: 'right'
  // })
  // ctx.save()
  // ctx.drawImage(
