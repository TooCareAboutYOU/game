...
import {towerObserver} from "./externalGame";
...
export const touchEventHandler = (engine) => {
  if (!gameStart) towerObserver.next({start: true})
...
export const addSuccessCount = (engine) => {
  ...
  towerObserver.next({success})
}

export const addFailedCount = (engine) => {
  const {setGameFailed, showWait} = engine.getVariable(constant.gameUserOption)
  ...
  if (failed >= 7) {
    towerObserver.next({failed})
    // engine.togglePaused()
    // if (showWait) showWait(failed)
    // if (setGameFailed) setGameFailed(failed)
...
export const pauseGame = (engine) => {
  engine.togglePaused()
}
export const addHeart = (engine) => {
  const lastFailedCount = engine.getVariable(constant.failedCount)
  if (lastFailedCount) {
    const failed = lastFailedCount - 1
    engine.setVariable(constant.failedCount, failed)
...
export const reduceHeart = (engine) => {
  const lastFailedCount = engine.getVariable(constant.failedCount)
  const failed = lastFailedCount + 1
  engine.setVariable(constant.failedCount, failed)
}